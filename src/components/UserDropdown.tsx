...
export function UserDropdown({ userName, userEmail, userImage, userId }: Props) {
  ...
  return (
    <div ref={dropdownRef} className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-64 bg-dark-surface border border-dark-border rounded-lg shadow-xl z-50 overflow-hidden">
          {/* User info header */}
          <div className="px-4 py-3 border-b border-dark-border">
            <p className="text-sm font-medium text-dark-text truncate">{userName}</p>
            <p className="text-xs text-dark-muted truncate">{userEmail}</p>
          </div>
          ...
          <div className="py-1">
            <Link
              ...
              className="flex items-center gap-3 px-4 py-2 text-sm text-dark-text hover:bg-dark-hover transition-colors"
            >
