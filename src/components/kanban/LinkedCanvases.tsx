import { useState } from "react";
import { useQuery, useMutation } from "convex/react";
import { api } from "convex/_generated/api";
import type { Id } from "convex/_generated/dataModel";
import { Link } from "@tanstack/react-router";
import { useConvexUser } from "@/hooks/useConvexUser";

interface Props {
  cardId: Id<"cards">;
  boardId: Id<"boards">;
  canEdit: boolean;
}

function CanvasIcon({ className }: { className: string }) {
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth={2}
        d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"
      />
    </svg>
  );
}

export function LinkedCanvases({ cardId, boardId, canEdit }: Props) {
  const [isSearching, setIsSearching] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const { session } = useConvexUser();

  const linkedCanvases = useQuery(api.canvasLinks.listByCard, session ? { cardId } : "skip");

  const searchResults = useQuery(
    api.canvases.search,
    isSearching && searchQuery.length > 0 && session ? { boardId, query: searchQuery } : "skip"
  );

  const linkCanvas = useMutation(api.canvasLinks.link);
  const unlinkCanvas = useMutation(api.canvasLinks.unlink);

  const handleLink = async (canvasId: Id<"canvases">) => {
    try {
      await linkCanvas({ cardId, canvasId });
      setIsSearching(false);
      setSearchQuery("");
    } catch (error) {
      console.error("Failed to link canvas:", error);
    }
  };

  const handleUnlink = async (canvasId: Id<"canvases">) => {
    try {
      await unlinkCanvas({ cardId, canvasId });
    } catch (error) {
      console.error("Failed to unlink canvas:", error);
    }
  };

  const filteredResults = searchResults?.filter(
    (canvas) => !linkedCanvases?.some((linked) => linked._id === canvas._id)
  );

  return (
    <div className="space-y-2">
      {linkedCanvases && linkedCanvases.length > 0 ? (
        <div className="space-y-1">
          {linkedCanvases.map((canvas) => (
            <div key={canvas._id} className="flex items-center gap-2 group">
              <Link
                to="/boards/$boardId/canvas/$canvasId"
                params={{ boardId, canvasId: canvas._id }}
                className="flex-1 flex items-center gap-2 px-2 py-1.5 text-sm bg-dark-bg rounded hover:bg-dark-hover transition-colors min-w-0"
              >
                <CanvasIcon className="w-4 h-4 flex-shrink-0 text-dark-muted" />
                <span className="truncate">{canvas.name}</span>
              </Link>
              {canEdit && (
                <button
                  onClick={() => handleUnlink(canvas._id)}
                  className="p-1 text-dark-muted hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
                  title="Unlink canvas"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M6 18L18 6M6 6l12 12"
                    />
                  </svg>
                </button>
              )}
            </div>
          ))}
        </div>
      ) : (
        <p className="text-xs text-dark-muted">No linked canvases</p>
      )}

      {canEdit && (
        <div className="relative">
          {isSearching ? (
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search canvases..."
                  className="flex-1 px-2 py-1.5 text-sm bg-dark-bg border border-dark-border rounded focus:outline-none focus:ring-1 focus:ring-accent"
                  autoFocus
                  onKeyDown={(e) => {
                    if (e.key === "Escape") {
                      setIsSearching(false);
                      setSearchQuery("");
                    }
                  }}
                />
                <button
                  onClick={() => {
                    setIsSearching(false);
                    setSearchQuery("");
                  }}
                  className="p-1 text-dark-muted hover:text-dark-text"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M6 18L18 6M6 6l12 12"
                    />
                  </svg>
                </button>
              </div>

              {filteredResults && filteredResults.length > 0 ? (
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {filteredResults.map((canvas) => (
                    <button
                      key={canvas._id}
                      onClick={() => handleLink(canvas._id)}
                      className="w-full flex items-center gap-2 px-2 py-1.5 text-sm text-left bg-dark-bg rounded hover:bg-dark-hover transition-colors"
                    >
                      <CanvasIcon className="w-4 h-4 flex-shrink-0 text-dark-muted" />
                      <span className="truncate">{canvas.name}</span>
                    </button>
                  ))}
                </div>
              ) : searchQuery.length > 0 ? (
                <p className="text-xs text-dark-muted px-2">No canvases found</p>
              ) : null}
            </div>
          ) : (
            <button
              onClick={() => setIsSearching(true)}
              className="w-full flex items-center justify-center gap-2 px-3 py-1.5 text-sm text-dark-muted hover:text-dark-text hover:bg-dark-hover border border-dashed border-dark-border rounded transition-colors"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
              </svg>
              Link Canvas
            </button>
          )}
        </div>
      )}
    </div>
  );
}
