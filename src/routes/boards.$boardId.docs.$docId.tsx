...
function DocumentEditorPage() {
  ...
  const handleDelete = async () => {
    if (!userEmail || !window.confirm("Are you sure you want to delete this document?")) return;
  ...
  if (document === null) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-3.5rem)]">
        <h1 className="text-xl font-medium text-dark-muted mb-4">Document not found</h1>
        <Link
    ...
    <div className="h-[calc(100vh-3.5rem)] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="w-full px-8 py-8">
          ...
          {document.linkedCards && document.linkedCards.filter(Boolean).length > 0 && (
            <div className="mt-8 pt-6 border-t border-dark-border">
              <h3 className="text-sm font-medium text-dark-muted mb-3">Linked Cards</h3>
              <div className="flex flex-wrap gap-2">
                {document.linkedCards.filter((card): card is NonNullable<typeof card> => card !== null).map((card) => (
                  <Link
                    key={card.id}
                  ...
                  >
                    <span className="text-accent font-mono text-xs">{card.slug}</span>
